 ...
 * <p>
 * Each record consists of a name, length, and a set of notes, each with a pitch
 * and rhythm. The length corrosponds to how many notes are in a record. The
 * value of the pitch corrosponds to the frequency of a note: 1 is the lowest
 * value, 13 is the highest value. The "rhythm" is an inverse measure of
 * durration: 4 means it the note is a quarter note, 1 means it is a whole note,
 * etc.
 * 
...
public class RecordHandler {
	 ...
	 * 
	 * @param args[] The arguments that can be passed into the program when prompted
	 *               to run.
	 */
	...
	private void load() {
		try {
			DataInputStream stream = new DataInputStream(new FileInputStream("src/records/Records.jm"));
			int i;
			int j;
			String name;
	...
	public void save() {
		...
		try {
			DataOutputStream stream = new DataOutputStream(
					new BufferedOutputStream(new FileOutputStream("records/" + saveName() + ".jm")));
	...
	/**
	 * Adds a record. The record is not saved in the Records.jm file until save() is
	 * called.
	 * 
	...
	/**
	 * Removes a record. This change is not saved in the records.jm file until
	 * save() is called.
	 * 
	...
	/**
	 * Returns the name of a record at a given index. Helpful for displaying the
	...
	/**
	 * Returns the length of the current record. Basically, it returns how many
	 * notes are in the current record.
	 * 
	...
	/**
	 * Sets the length of the current record. Basically, it sets how many notes the
	 * record will have. The length can not be less than 1.
	 * 
	...
	/**
	 * Begins recording. Recording allows for the pitch of each note in the current
	 * record to be set from beginning to end.
	 */
	...
	/**
	 * If recording, sets the pitch of the next note in the current record. If the
	 * end of the record is reached, stops recording.
	 * 
	...
	/**
	 * Begins Recalling. "Recalling" allows for the rhythm and pitch of each note in
	 * the current record to be returned from beginning to end.
	 */
	...
	/**
	 * If recalling, returns the pitch of the next note in the current record. If
	 * the end of the record has been reched, ends recording. If the next pitch is
	 * not returned, returns -1.
	 * 
	...
	/**
	 * If recalling, returns the rhythm of this note in the current record. Else:
	 * returns -1.
	 * 
